...
//    }
    ...
    //todo ori begin
    public static double[] stringToArrayDouble(String str, String delimiter, int n) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                int sign = currStr.indexOf("inf");
                if (sign != -1) {
                    ret[i] = repl;
                    continue;
                }
                sign = currStr.indexOf("e");
                if (sign != -1) {
                    ret[i] = STLToDoble(currStr);
                } else {
                    ret[i] = Double.parseDouble(strs[i]);
                }
            }
            ...
            System.out.println("model data input string error!");
    ...
    public static double STLToDoble(String stl) {
        double number = 0;
        BigDecimal bd = new BigDecimal(stl);
